...
if (!isset($_SESSION['documento'])) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insertado = insertarLogExpediente($conexion, $idExpediente, $anioExpediente, $idAreaInicio, $idAreaFin, 'DESARCHIVADO-TOMADO', $comentario, $usuario);

    if($actualizado1 && $actualizado2 && $insertado){
      // Se confirman los cambios realizados en la transacción
      mysqli_commit($conexion);
      echo json_encode(array('mensaje' => "Expediente N°".$idExpediente." desarchivado", 'salida' => '0'));
    }
    else{
...
// Función para actualizar el estado de la bandeja, pasa a estado "TOMADO"
function actualizarEstadoBandeja($conexion, $idExpediente, $anioExpediente) {
  $sql = "UPDATE bandejas SET estado='TOMADO' WHERE id_expediente=? and anio=?";
  $stmt = mysqli_prepare($conexion, $sql);
  mysqli_stmt_bind_param($stmt, "ii", $idExpediente, $anioExpediente);
  return mysqli_stmt_execute($stmt);
}

// Función para actualizar el estado del expediente, pasa a archivado = 1
function actualizarEstadoExpediente($conexion, $idExpediente, $anioExpediente) {
  $sql = "UPDATE expedientes SET archivado='1' WHERE id_expediente=? and anio=?";
  $stmt = mysqli_prepare($conexion, $sql);
  mysqli_stmt_bind_param($stmt, "ii", $idExpediente, $anioExpediente);
  return mysqli_stmt_execute($stmt);
}
